...
def compile_fantome(name="fantome", extension="so", usage="julia"):
	"""Compile keplerder module
	...
	print(f"Compiling {name}.{extension} ...")
	if usage=="julia":
		os.system(f"gfortran linalg.f90 orbitalelements.f90 keplerder.f90 fantome.f90 -o {name}.{extension} -shared -fPIC")
	elif usage=="python":
		...
		os.system(f"gfortran linalg.f90 orbitalelements.f90 keplerder.f90 fantome.f90 -o {name}.{extension}")
	else:
		raise ValueError("Set usage to None if not using \"julia\" or \"python\"!")
	return
